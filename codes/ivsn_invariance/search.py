...
def render_trial_from_struct(trial: Trial, args):
    ...
    search_img = render_search_display(Path(trial.target_path), [Path(p) for p in trial.distractor_paths], trial.target_position, target_spec, distractor_specs, args)
...
def run_trial(model: BaseAttentionModel, trial: Trial, args) -> dict:
    ...
    return {'condition_name': trial.condition_name, 'condition_group': trial.condition_group, 'condition_value': trial.condition_value, 'unique_id': trial.unique_id, 'repeat_id': trial.repeat_id, 'trial_type': trial.trial_type, 'target_category': trial.target_category, 'target_position': trial.target_position, 'n_fixations': len(fix_positions), 'found': bool(found), 'found_within_3_fixations': found_within_3, 'fixation_positions': fix_positions, 'fixation_centers': fix_centers, 'scores_initial': [float(x) for x in scores.tolist()], 'score_history': score_history, 'score_target': score_target, 'score_max_distractor': score_max_distractor, 'score_mean_distractor': score_mean_distractor, 'score_margin': score_margin, 'target_rank': target_rank, 'is_top1': is_top1, 'p_target': p_target, 'cue_path': trial.cue_path, 'target_path': trial.target_path, 'distractor_paths': trial.distractor_paths, 'cue_transform': trial.cue_transform, 'target_transform': trial.target_transform, 'distractor_transforms': trial.distractor_transforms}
