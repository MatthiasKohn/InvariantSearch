...
run_set () {
  ...
  python "$SCRIPT" --data-root "$DATA_ROOT" --out-dir "$RESULT_ROOT/conv_gist_${NOBJ}obj_baseline" \
    --n-objects "$NOBJ" --transform-mode scale --scale-values 1.0 "${COMMON_ARGS[@]}"

  python "$SCRIPT" --data-root "$DATA_ROOT" --out-dir "$RESULT_ROOT/conv_gist_${NOBJ}obj_rotation" \
    --n-objects "$NOBJ" --transform-mode rotation --rotation-values 0 30 60 90 120 150 180 "${COMMON_ARGS[@]}"

  python "$SCRIPT" --data-root "$DATA_ROOT" --out-dir "$RESULT_ROOT/conv_gist_${NOBJ}obj_scale" \
    --n-objects "$NOBJ" --transform-mode scale --scale-values 0.5 0.75 1.0 1.25 1.5 "${COMMON_ARGS[@]}"

  python "$SCRIPT" --data-root "$DATA_ROOT" --out-dir "$RESULT_ROOT/conv_gist_${NOBJ}obj_skewx" \
    --n-objects "$NOBJ" --transform-mode skew_x --skew-values -20 -10 0 10 20 "${COMMON_ARGS[@]}"

  python "$SCRIPT" --data-root "$DATA_ROOT" --out-dir "$RESULT_ROOT/conv_gist_${NOBJ}obj_skewy" \
    --n-objects "$NOBJ" --transform-mode skew_y --skew-values -20 -10 0 10 20 "${COMMON_ARGS[@]}"

  python "$SCRIPT" --data-root "$DATA_ROOT" --out-dir "$RESULT_ROOT/conv_gist_${NOBJ}obj_noise" \
    --n-objects "$NOBJ" --transform-mode noise --noise-values 0.0 0.03 0.06 0.09 0.12 "${COMMON_ARGS[@]}"

  python "$SCRIPT" --data-root "$DATA_ROOT" --out-dir "$RESULT_ROOT/conv_gist_${NOBJ}obj_blur" \
